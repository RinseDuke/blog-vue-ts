...
describe('HomeView liquid glass content contract', () => {
  ...
  it('uses a rhythmic desktop card grid that collapses to one column on mobile', () => {
    ...
    expect(postCardSource).toContain('class="post-card"')
    expect(postCardSource).not.toContain('class="post-card glass-surface"')
    const postCardBlock = extractBlock(postCardSource, '.post-card')
    expect(postCardBlock).toContain('background: var(--post-card-bg);')
    expect(postCardBlock).toContain('border: 1px solid var(--post-card-border);')
    expect(postCardBlock).not.toContain('backdrop-filter:')
    expect(postCardBlock).not.toContain('-webkit-backdrop-filter:')
